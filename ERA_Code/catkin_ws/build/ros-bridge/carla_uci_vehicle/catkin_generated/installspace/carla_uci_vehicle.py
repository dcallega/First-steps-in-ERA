...
"""
base class for spawning a UCI Vehicle in ROS
...
class CarlaUCIVehicle(object):

    """
    Handles the spawning of the UCI vehicle and its sensors

    Derive from this class and implement method sensors()
    """

    def __init__(self):
        rospy.init_node('uci_vehicle', anonymous=True)
        ...
        self.role_name = rospy.get_param('~role_name', 'uci_vehicle')
        # check argument and set spawn_point
        spawn_point_param = rospy.get_param('~spawn_point')
        if spawn_point_param and rospy.get_param('~spawn_uci_vehicle'):
    ...
    def restart(self):
        ...
        # Spawn the vehicle.
        if not rospy.get_param('~spawn_uci_vehicle'):
    ...
    def setup_sensors(self, sensors):
        """
        Create the sensors defined by the user and attach them to the uci-vehicle
    ...
    def destroy(self):
        """
        destroy the current UCI vehicle and its sensors
        """
    ...
    def run(self):
        ...
        try:
            ...
            raise e
        rospy.loginfo("CARLA world available. Spawn UCI vehicle...")
...
def main():
    ...
    main function
    """
    uci_vehicle = CarlaUCIVehicle()
    try:
        uci_vehicle.run()
    finally:
        if uci_vehicle is not None:
            uci_vehicle.destroy()
